...
namespace diffpng
{
    ...
    static void AdobeRGBToXYZ(v8f r, v8f g, v8f b,
            v8f &x, v8f &y, v8f &z)
    {
    ...
    static void XYZToLAB(v8f x, v8f y, v8f z, v8f &L, v8f &A, v8f &B)
    {
    ...
    bool Yee_Compare_Engine(CompareArgs &args)
    {
        ...
        vector<float> aX(dim),aY(dim),aZ(dim);
        vector<float> bX(dim),bY(dim),bZ(dim);
        vector<float> aLum(dim),bLum(dim);
        vector<float> aA(dim),bA(dim),aB(dim),bB(dim);
        ...
        {
#ifdef __AVX2__
            for (unsigned x = 0u; x < w; x += 8) {
                const unsigned i = x + y * w;
                const unsigned border = w * h - i > 8 ? 8 : w * h - i;
                v8f rs, gs, bs;
                if (border >= 8) {
                    rs = v8f{
                        static_cast<float>(args.ImgA->Get_Red(i)),
                            static_cast<float>(args.ImgA->Get_Red(i+1)),
                            static_cast<float>(args.ImgA->Get_Red(i+2)),
                            static_cast<float>(args.ImgA->Get_Red(i+3)),
                            static_cast<float>(args.ImgA->Get_Red(i+4)),
                            static_cast<float>(args.ImgA->Get_Red(i+5)),
                            static_cast<float>(args.ImgA->Get_Red(i+6)),
                            static_cast<float>(args.ImgA->Get_Red(i+7)),
                    };
                    gs = v8f{
                        static_cast<float>(args.ImgA->Get_Green(i)),
                            static_cast<float>(args.ImgA->Get_Green(i+1)),
                            static_cast<float>(args.ImgA->Get_Green(i+2)),
                            static_cast<float>(args.ImgA->Get_Green(i+3)),
                            static_cast<float>(args.ImgA->Get_Green(i+4)),
                            static_cast<float>(args.ImgA->Get_Green(i+5)),
                            static_cast<float>(args.ImgA->Get_Green(i+6)),
                            static_cast<float>(args.ImgA->Get_Green(i+7)),
                    };
                    bs = v8f{
                        static_cast<float>(args.ImgA->Get_Blue(i)),
                            static_cast<float>(args.ImgA->Get_Blue(i+1)),
                            static_cast<float>(args.ImgA->Get_Blue(i+2)),
                            static_cast<float>(args.ImgA->Get_Blue(i+3)),
                            static_cast<float>(args.ImgA->Get_Blue(i+4)),
                            static_cast<float>(args.ImgA->Get_Blue(i+5)),
                            static_cast<float>(args.ImgA->Get_Blue(i+6)),
                            static_cast<float>(args.ImgA->Get_Blue(i+7)),
                    };
                } else {
                    rs = 11;
                    gs = 12;
                ...
                if (border >= 8) {
                    rs = v8f{
                        static_cast<float>(args.ImgB->Get_Red(i)),
                            static_cast<float>(args.ImgB->Get_Red(i+1)),
                            static_cast<float>(args.ImgB->Get_Red(i+2)),
                            static_cast<float>(args.ImgB->Get_Red(i+3)),
                            static_cast<float>(args.ImgB->Get_Red(i+4)),
                            static_cast<float>(args.ImgB->Get_Red(i+5)),
                            static_cast<float>(args.ImgB->Get_Red(i+6)),
                            static_cast<float>(args.ImgB->Get_Red(i+7)),
                    };
                    gs = v8f{
                        static_cast<float>(args.ImgB->Get_Green(i)),
                            static_cast<float>(args.ImgB->Get_Green(i+1)),
                            static_cast<float>(args.ImgB->Get_Green(i+2)),
                            static_cast<float>(args.ImgB->Get_Green(i+3)),
                            static_cast<float>(args.ImgB->Get_Green(i+4)),
                            static_cast<float>(args.ImgB->Get_Green(i+5)),
                            static_cast<float>(args.ImgB->Get_Green(i+6)),
                            static_cast<float>(args.ImgB->Get_Green(i+7)),
                    };
                    bs = v8f{
                        static_cast<float>(args.ImgB->Get_Blue(i)),
                            static_cast<float>(args.ImgB->Get_Blue(i+1)),
                            static_cast<float>(args.ImgB->Get_Blue(i+2)),
                            static_cast<float>(args.ImgB->Get_Blue(i+3)),
                            static_cast<float>(args.ImgB->Get_Blue(i+4)),
                            static_cast<float>(args.ImgB->Get_Blue(i+5)),
                            static_cast<float>(args.ImgB->Get_Blue(i+6)),
                            static_cast<float>(args.ImgB->Get_Blue(i+7)),
                    };
                } else {
                    rs = 11;
                    gs = 12;
        ...
        std::vector<float> F_freq(args.MaxPyramidLevels - 2);
        ...
        for (unsigned y = 0u; y < h; y++)
        {
            for (unsigned x = 0u; x < w; x++)
            {
                ...
                std::vector<float> F_mask(args.MaxPyramidLevels - 2);
    ...
    bool LevelClimberCompare(CompareArgs &args) {
        bool test = false;
        std::chrono::milliseconds ms_begin = std::chrono::duration_cast< std::chrono::milliseconds >(
                std::chrono::system_clock::now().time_since_epoch());

        //std::cout << ms.count() << std::endl;

        while (test==false && args.MaxPyramidLevels<args.FinalMaxPyramidLevels) {
            std::chrono::milliseconds ms_prev = std::chrono::duration_cast< std::chrono::milliseconds >(
                    std::chrono::system_clock::now().time_since_epoch());
            ...
            std::chrono::milliseconds ms_next = std::chrono::duration_cast< std::chrono::milliseconds >(
                    std::chrono::system_clock::now().time_since_epoch());
            std::cout << "------------------------ time elapsed: " << ms_next.count() - ms_prev.count() << std::endl;
            if (test==false) {
        ...
        std::chrono::milliseconds ms_end = std::chrono::duration_cast< std::chrono::milliseconds >(
                std::chrono::system_clock::now().time_since_epoch());
        std::cout << "------------------------ time elapsed: " << ms_end.count() - ms_begin.count() << std::endl;
        std::cout << "--------------------- The comparision result is:" << test << std::endl;
...
*/
    ...
    static constexpr int tp_size = 64;
    static ThreadPool tp(tp_size);
    ...
    std::vector<bool> LevelClimberCompareMulti(std::vector<CompareArgs> &args) {
        ...
        std::transform(args.begin(), args.end(), fts.begin(),
                [](CompareArgs& a) -> std::shared_future<bool>
                {return tp.enqueue(&diffpng::LevelClimberCompare, std::ref(a));});
        //{return std::async(std::launch::async, &diffpng::LevelClimberCompare, std::ref(a)).share();});
...
int main(int argc, char **argv)
{
    ...
    for (int j = 0; j < 1; ++j) {
        for (int i = 0; i < 1; ++i) {
            ...
            std::for_each(argss.begin(), argss.end(), [&argc, &argv](diffpng::CompareArgs& arg){
                    arg.Parse_Args(argc, argv);
                    });
            diffpng::CompareArgs& args = argss[0];
#endif
#ifndef TESTMULTI
