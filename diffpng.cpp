...
class RGBAImage {
  public:
    ...
    RGBAImage(unsigned int w, unsigned int h, const string &name = "") : Width(w), Height(h), Name(name), Data(w * h) {}
...
string copyright("diffpng version 2014,\n\
...
template <typename T> static T lexical_cast(const string &input)
{
...
static void AdobeRGBToXYZ(v8f r, v8f g, v8f b, v8f &x, v8f &y, v8f &z)
{
...
#define calc_f_from_r(r, f)                                                                                            \
    if ((r) > epsilon) {                                                                                               \
        (f) = powf((r), 1.0f / 3.0f);                                                                                  \
    }                                                                                                                  \
    else {                                                                                                             \
        (f) = (kappa * (r) + 16.0f) / 116.0f;                                                                          \
    }
...
{
    if ((args.ImgA->Get_Width() != args.ImgB->Get_Width()) or (args.ImgA->Get_Height() != args.ImgB->Get_Height())) {
...
#define powf256(base, exponatial)                                                                                      \
    exp256_ps(static_cast<__m256>(v8f(log256_ps(static_cast<__m256>(base)) * (exponatial))))
            ...
            // gs = exp256_ps(static_cast<__m256>(v8f(log256_ps(static_cast<__m256>(gs / 255.0f)) * args.Gamma)));
            // bs = exp256_ps(static_cast<__m256>(v8f(log256_ps(static_cast<__m256>(bs / 255.0f)) * args.Gamma)));
            rs = powf256(rs / 255.0f, args.Gamma);
            gs = powf256(gs / 255.0f, args.Gamma);
            bs = powf256(bs / 255.0f, args.Gamma);
            v8f ax, ay, az;
...
bool ShiftAndTest(CompareArgs &args, RGBAImage &originalB, int xshift, int yshift)
{
    ...
    if (args.ImgDiff)
        args.ImgDiff->WriteToFile(args.ImgDiff->Get_Name() + ".diffshift1.png");
    cout << "result: " << test << "\n";
...
bool LevelClimberCompare(CompareArgs &args)
{
    ...
    while (test == false && args.MaxPyramidLevels < args.FinalMaxPyramidLevels) {
        ...
        if (test == false) {
            ...
            if (args.MaxPyramidLevels < args.FinalMaxPyramidLevels)
                cout << "Retesting.\n\n";
        }
        else {
            return true;
    ...
    if (test == false) {
        ...
        if (ShiftAndTest(args, originalB, -2, 0))
            return true;
        if (ShiftAndTest(args, originalB, 2, 0))
            return true;
        if (ShiftAndTest(args, originalB, 0, -2))
            return true;
        if (ShiftAndTest(args, originalB, 0, 2))
            return true;

        if (ShiftAndTest(args, originalB, -1, 0))
            return true;
        if (ShiftAndTest(args, originalB, 1, 0))
            return true;
        if (ShiftAndTest(args, originalB, 0, -1))
            return true;
        if (ShiftAndTest(args, originalB, 0, 1))
            return true;
...
std::vector<bool> LevelClimberCompareMulti(std::vector<CompareArgs> &args)
{
    ...
    std::transform(args.begin(), args.end(), fts.begin(), [](CompareArgs &a) -> std::shared_future<bool> {
    ...
    std::transform(fts.begin(), fts.end(), ret.begin(), [](std::shared_future<bool> ft) -> bool { return ft.get(); });
    return ret;
}
...
int main(int argc, char **argv)
{
    ...
    for (int j = 0; j < 1; ++j) {
        for (int i = 0; i < 1; ++i) {
            ...
            std::for_each(argss.begin(), argss.end(),
                          [&argc, &argv](diffpng::CompareArgs &arg) { arg.Parse_Args(argc, argv); });
