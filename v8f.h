...
class v8f {
    ...
    public:
        ...
        v8f(float f0, float f1, float f2, float f3, float f4, float f5, float f6, float f7)
        {
...
STI v8f C_F()
{
    static const union {
        int f[8];
        v8f ymm;
    } u = {{(float) i0, (float) i1, (float) i2, (float) i3, (float) i4, (float) i5, (float) i6, (float) i7}};
    return u.ymm;
}
...
STI v8f C_F()
{
    ...
    static const union {
        float f[8];
        v8f ymm;
    } u = {{(float) i0 / i1, (float) i0 / i1, (float) i0 / i1, (float) i0 / i1, (float) i0 / i1, (float) i0 / i1,
        (float) i0 / i1, (float) i0 / i1}};
...
STI v8f OP !(v8f const &a)
{
    return _mm256_cmp_ps(a.v, _mm256_setzero_ps(), 0);
}
...
STI std::ostream &OP << (std::ostream & output, const v8f &p)
{
    output << "v8f: [";
    for (int i = 0; i < (8); ++i)
        output << p.v[i] << ",";
...
STI int compare(Pv8fab)
{
    ...
    return static_cast<int>(horizontal_add(if_select(abs(a - b) > epsilon, 1, 0)));
    /*
       for (int i = 0; i < (8); ++i) {
